...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        for credit in credits_context:
            if credit["subject_id_list_that_using_this_credit"] == [subject_id]:
                credit = credit["id"]
                break
        ...
        for score_data in scores_context:
            if score_data["student_first_name"] == student_first_name and score_data["student_last_name"] == student_last_name and score_data["subject_id"] == subject_id:
        ...
        new_score_data = {
            ...
            "score": score,
            "credit": credit}
        scores_context.append(new_score_data)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        try:
            student = StudentSubjectsScore.objects.get(id=student_id)
        except StudentSubjectsScore.DoesNotExist:
                    return Response(status=status.HTTP_404_NOT_FOUND)
        
        subject_details = Subjects.objects.filter(student=student)
        ...
        for subject in subject_details:
            ...
            grade_point = credits * (ord(grade[0]) - ord('A') + 4)
            grade_average += grade_point
            total_credit += credits

        if total_credit !=0:
            grade_average = grade_average / total_credit            
        ...
        for school_data in data_pattern:
            ...
            for class_data in school_data.values():
                if isinstance(class_data, dict):
                    for teacher_data in class_data.values():
                        teacher_name = list(teacher_data.key())[0]
                        hierarchy_data = {
                            "school": school_name,
                            "teacher": teacher_name,
                            "class": list(class_data.keys())[0]
                        }

                        student = []
                        for student_data in teacher_data[teacher_name]:
                            if "Student" in student_data:
                                student.append(student_data["Student"])
                            elif "Head of the room" in student_data:
                                student.append(student_data["Head of the room"])
                        hierarchy_data["students"] = student
                        your_result.append(hierarchy_data)
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        def build_structure(structure):
            ...
            if "sub" in structure:
                result["sub"] = [build_structure(sub) for sub in structure["sub"]]
            return result

        your_result = [build_structure(item) for item in data_pattern]
